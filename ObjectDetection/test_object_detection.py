...
import object_detection as detector

log = logging.getLogger('Test.detector')
...
def test_detector_get(client):
    '''This tests the get function by sending a get request. The url is defined 
    from the base URL of the app. In this case detector resource was created at 
    /detector. No localhost or ports needed'''
    log.info("Testing a get")
    response = client.get("/detector/42")
    assert response.status_code == 200


def test_detector_put(client):
    '''Test adding a user'''
    ...
    response = client.put("/detector/99",json={"arg0" : "foo", "arg1" : "bar"})
